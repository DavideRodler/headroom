...
Headroom's MCP server exposes compression, retrieval, and observability as tools that any MCP-compatible AI coding tool can call -- Claude Code, Cursor, Codex, Pi Code Agent, and more. No proxy required.
...
| Pi Code Agent | Supported via pi-mcp-adapter | Add `headroom mcp serve` to `.pi/mcp.json` |
